...
def _real_transcribe(
    ...
    try:
        ...
        if not url and not vad_ran:
            logger.debug(
                "Transcription skipped: no audio URL configured."
            )
            return None
        ...
        logger.info(
            "Whisper not installed, using stub fallback"
        )
        return None
...
def _real_detect_background_voices(
    ...
    try:
        ...
        url = audio_url or os.environ.get(
            "AUDIO_STREAM_URL",
            "",
        ).strip()
            ...
            [
                {
                    ...
                    "content": (
                        ...
                        "the transcript. "
                        "Detect: reading from script, "
                        "robotic/unnatural responses, "
                        "inconsistent knowledge, or possible use of "
                        "AI assistants. "
                        "Return a JSON object with keys: suspicious (bool), "
                        "pattern_type (str or null), confidence (float 0-1), "
                        "details (object)."
                    ),
                },
                {
...
def run_audio_analysis(session_id: str) -> dict[str, Any]:
    ...
    logger.info(
        f"Starting audio analysis for session {session_id}"
    )
    ...
    logger.info(
        f"Audio analysis completed for session {session_id}: {results}"
    )
...
def transcribe_speech(
    ...
    logger.info(
        f"Transcribing audio for session {session_id}"
    )
...
def detect_background_voices(
    ...
    logger.info(
        f"Detecting background voices for session {session_id}"
    )
...
def detect_suspicious_conversation(
    ...
    logger.info(
        f"Detecting suspicious conversations for session {session_id}"
    )
