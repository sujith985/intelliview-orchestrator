...
- Interview turn-taking state tracking
...
from workers.turn_taking import InterviewState, TurnTakingStateMachine
...
def _get_audio_duration(
    ...
    segment's end timestamp as a best-effort estimate.
    """
    try:
        import wave
...
def run_audio_analysis(session_id: str) -> dict[str, Any]:
    """Execute audio analysis pipeline with turn-taking state tracking."""
    ...
    # B5: initialize the interview turn-taking state machine.
    turn_machine = TurnTakingStateMachine()

    logger.info(
        "[VOICE] session=%s state=%s",
        session_id,
        turn_machine.state.value,
    )

    # Question playback state.
    turn_machine.transition(
    ...
    # Wait for candidate speech to begin/end.
    turn_machine.transition(
    ...
    # Candidate speech is now being listened to.
    turn_machine.transition(
    ...
    # Speech has ended; begin transcription.
    turn_machine.transition(
    ...
    # Transcription completed; advance the interview turn.
    turn_machine.transition(
    ...
    results = {
        ...
        "risk_score": 0.0,
        "turn_state": turn_machine.state.value,
    }
...
def transcribe_speech(session_id: str) -> dict[str, Any]:
    ...
    text = (
        ...
        if silence
            ...
            "I have five years of experience building distributed "
            "systems in Python and Go. Recently I led a migration "
            "from a monolith to Celery-backed workers."
        )
    )
